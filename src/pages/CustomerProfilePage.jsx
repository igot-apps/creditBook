...
import { AddCustomerModal } from "../components/customer/AddCustomerModal"; // 👈 NEW: Import Edit Modal
              ...
              <div key={tx.id} className="space-y-2">
                <button onClick={() => onView(tx)} className={`w-full text-left p-3 rounded-xl border transition-all active:scale-[0.98] ${
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      <div className={`p-1 rounded-full ${isInvalid ? (isCancelled ? 'bg-red-100 dark:bg-red-900/40' : 'bg-yellow-100 dark:bg-yellow-900/40') : (tx.type === 'payment' ? 'bg-green-100 dark:bg-green-900/30' : 'bg-green-100 dark:bg-green-900/30')}`}>
                        {getTimelineIcon(tx)}
                      </div>
                      <div>
                        <p className={`font-bold text-sm ${isInvalid ? 'text-gray-500 line-through' : 'text-gray-900 dark:text-white'}`}>
                          {tx.type === 'payment' ? 'Payment' : tx.type === 'purchase' ? 'Purchase' : 'Sale'}
                        </p>
                        <p className="text-[10px] text-gray-500 dark:text-gray-400">{formatDate(tx.date || tx.createdAt).split(',')[0]}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      {tx.type === 'sale' && tx.paid > 0 && !isInvalid && <p className="text-[10px] text-green-600 dark:text-green-400">Paid: {formatCurrency(tx.paid, currency)}</p>}
                    </div>
                  </div>
...
const ReceiptModal = ({ tx, onClose, onViewTx, onFix, onCancel, currency, activePayments }) => {
  ...
  const subsequentPayments = useMemo(() => {
    if (tx.type !== 'sale' || !activePayments) return [];
      ...
      <div className="bg-white dark:bg-gray-900 w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white dark:bg-gray-900 p-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center z-10">
          <h3 className="font-bold text-lg text-gray-900 dark:text-white flex items-center gap-2">
            <FileText size={20} className={tx.type === 'payment' ? "text-blue-600" : "text-green-600"} /> 
            {tx.type === 'payment' ? "Payment Receipt" : "Sale Receipt"}
          </h3>
        ...
        <div className="p-5 space-y-4">
          <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-xl space-y-3">
            ...
            {tx.type === 'payment' ? (
              <div className="flex justify-between text-lg font-bold"><span className="text-gray-700 dark:text-gray-300">Amount Received</span><span className="text-green-600 dark:text-green-400">{formatCurrency(tx.paid, currency)}</span></div>
            ) : (
              <>
                <div className="flex justify-between text-sm"><span className="text-gray-500 dark:text-gray-400">Total Sale</span><span className="font-bold text-gray-900 dark:text-white">{formatCurrency(tx.amount, currency)}</span></div>
                <div className="flex justify-between text-sm"><span className="text-gray-500 dark:text-gray-400">Paid Upfront</span><span className="font-bold text-green-600 dark:text-green-400">{formatCurrency(tx.paid, currency)}</span></div>
          ...
          {tx.status === 'active' && (
            <div className="pt-2 space-y-2">
              <button onClick={() => onFix(tx)} className="w-full bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-800 font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition"><Edit3 size={18} /> Fix {tx.type === 'payment' ? 'Payment' : 'Sale'}</button>
              <button onClick={() => onCancel(tx)} className="w-full bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition"><Ban size={18} /> Cancel {tx.type === 'payment' ? 'Payment' : 'Sale'}</button>
            </div>
          )}
        </div>
      </div>
...
export const CustomerProfilePage = () => {
  ...
  const getTrueOutstanding = (sale) => {
    const allocated = activePayments.reduce((sum, p) => {
      if (p.allocations) {
        const alloc = p.allocations.find(a => a.transactionId === sale.id);
  ...
  const outstandingInvoices = useMemo(() => 
    history
      .filter(tx => tx.type === 'sale' && getTrueOutstanding(tx) > 0 && (tx.status === 'active' || !tx.status))
  ...
  const trueBalance = useMemo(() => {
    ...
    const totalPayments = history
      .filter(t => t.type === 'payment' && (t.status === 'active' || !t.status))
      .reduce((sum, t) => sum + (parseFloat(t.paid) || 0), 0);
      
    return totalSales - totalUpfront - totalPayments;
  }, [history]);

  const totalSales = history.reduce((sum, t) => sum + (t.amount > 0 && t.type === 'sale' && (t.status === 'active' || !t.status) ? t.amount : 0), 0);
  ...
  const generateMessage = (c) => `Hello ${c.name}, this is ${currentStore?.name || "Store"}. Please send your outstanding balance of ${formatCurrency(trueBalance, currency)} by the end of the week. Thank you!`;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-24">
      ...
      <div style={{ paddingTop: 'calc(env(safe-area-inset-top) + 4.5rem)' }} className="p-4 max-w-lg mx-auto space-y-5">
        ...
        {/* 👇 UPDATED: Pass onEdit to CustomerHeader */}
        <CustomerHeader 
      ...
      <AddCustomerModal 
        ...
        onSaved={() => {
          ...
          // Also refresh the global customers list in the background
          if (currentStore?.id) {
             CustomerService.getAll(currentStore.id).then(updated => {
               useStore.getState().setCustomers(updated);
             });
          }
        }} 
      />
