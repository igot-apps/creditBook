...
export const FollowUpsPage = () => {
  ...
  // Calculate follow-ups and categories
  ...
  const generateMessage = (c) => `Hello ${c.name}, this is a reminder from ${currentStore?.name || "Store"}. You have an outstanding balance of ${formatCurrency(c.balance, currency)}. Please visit us or send payment via MoMo. Thank you!`;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-24">
      {/* 👇 FIXED TOP BAR */}
      <TopBar title="Customers Owing" />
      
      {/* 👇 MAIN CONTENT - Increased top padding (5.5rem) for more breathing room */}
      <div style={{ paddingTop: 'calc(env(safe-area-inset-top) + 5.5rem)' }} className="p-4 max-w-lg mx-auto space-y-4">
        ...
        <div className="w-full overflow-x-auto flex gap-2 pb-2 scrollbar-hide">
          {filters.map(f => (
            <button 
              ...
              className={`flex-shrink-0 px-4 py-2.5 rounded-full text-xs sm:text-sm font-semibold whitespace-nowrap transition active:scale-95 ${
                filter === f.key 
          ...
          {filtered.length === 0 ? (
            ...
            <div key={c.id} className="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => openSMS(c.phone, generateMessage(c))} className="bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-semibold py-2.5 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition text-sm">
                  <MessageSquare size={16} /> SMS
                </button>
                <button onClick={() => openWhatsApp(c.phone, generateMessage(c))} className="bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 font-semibold py-2.5 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition text-sm">
                  <MessageCircle size={16} /> WhatsApp
                </button>
                <button onClick={() => openDialer(c.phone)} className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-semibold py-2.5 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition text-sm">
                  <Phone size={16} /> Call
                </button>
                <button onClick={() => handleMarkPaid(c.id)} className="bg-yellow-50 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 font-semibold py-2.5 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition text-sm">
