...
export const SupplierProfilePage = () => {
  const { currentStore, selectedSupplier, setSelectedSupplier, setView, setPrefillTransaction, setFixTransaction, showToast, triggerConfetti, lastScrollPosition, setLastScrollPosition } = useStore();
  ...
  const totalPurchases = history.reduce((sum, t) => sum + (t.amount > 0 && t.status !== 'cancelled' && t.status !== 'being_corrected' ? t.amount : 0), 0);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-24">
      ...
      <div style={{ paddingTop: 'calc(env(safe-area-inset-top) + 4.5rem)' }} className="p-4 max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm text-center border border-gray-100 dark:border-gray-700">
          <div className="w-20 h-20 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 rounded-full mx-auto mb-3 flex items-center justify-center text-3xl font-bold">{supplierData.name.charAt(0)}</div>
          <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{supplierData.name}</h3>
          <p className="text-gray-500 dark:text-gray-400 flex items-center justify-center gap-1 mt-1"><Phone size={14} /> {supplierData.phone}</p>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{supplierData.balance > 0 ? "Amount I Owe" : supplierData.balance < 0 ? "Overpaid (Credit)" : "All Paid Up"}</p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          <button onClick={handleRecordPurchase} className="bg-indigo-600 text-white p-3 rounded-xl flex flex-col items-center gap-1 active:scale-95 transition shadow-md"><FileText size={20} /> <span className="text-[10px] font-bold">Purchase</span></button>
          ...
          <button onClick={() => openWhatsApp(supplierData.phone, generateMessage(supplierData))} className="bg-green-500 text-white p-3 rounded-xl flex flex-col items-center gap-1 active:scale-95 transition shadow-md"><MessageCircle size={20} /> <span className="text-[10px] font-bold">WhatsApp</span></button>
          <button onClick={() => openSMS(supplierData.phone, generateMessage(supplierData))} className="bg-blue-500 text-white p-3 rounded-xl flex flex-col items-center gap-1 active:scale-95 transition shadow-md"><MessageSquare size={20} /> <span className="text-[10px] font-bold">SMS</span></button>
          <button onClick={() => openDialer(supplierData.phone)} className="col-span-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 p-3 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition"><Phone size={18} /> <span className="text-xs font-bold">Call Supplier</span></button>
            ...
            <div className="p-5 space-y-4">
              ...
              {viewingTransaction.status === 'active' && (
                <div className="pt-2 space-y-2">
                  <button onClick={() => handleFixTransaction(viewingTransaction)} className="w-full bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-800 font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition"><Edit3 size={18} /> Fix Purchase</button>
