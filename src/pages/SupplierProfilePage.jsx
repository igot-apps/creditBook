...
export const SupplierProfilePage = () => {
  ...
  // 👇 FIX: Fetch history separately using the new service
  useEffect(() => {
    if (selectedSupplier?.id) {
      SupplierService.getById(selectedSupplier.id).then(setSupplierData);
      SupplierService.getHistory(selectedSupplier.id).then(setHistory);
    }
  }, [selectedSupplier?.id]);

  if (!supplierData) return null;

  // Calculate stats from the fetched history array
  ...
  const executeCancelTransaction = async () => {
    ...
    try {
      await SupplierService.voidTransaction(viewingTransaction.id, cancelReason);
      const updated = await SupplierService.getById(supplierData.id);
      setSupplierData(updated);
      const updatedHistory = await SupplierService.getHistory(supplierData.id);
  ...
  const executeRedoTransaction = async () => {
    ...
    try {
      await SupplierService.redoTransaction(viewingTransaction.id);
      const updated = await SupplierService.getById(supplierData.id);
      setSupplierData(updated);
      const updatedHistory = await SupplierService.getHistory(supplierData.id);
      setHistory(updatedHistory);
              ...
              {viewingTransaction.items && viewingTransaction.items.length > 0 && (
                <div className="space-y-3">
                  <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-1">
                    Items Purchased ({viewingTransaction.items.length})
                  </p>
      ...
      {showRedoModal && (
        <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-sm w-full shadow-2xl p-6">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-4 text-sm">This will restore the canceled <span className="font-bold text-orange-600">{formatCurrency(viewingTransaction?.amount, currency)}</span> purchase and add it back to the balance.</p>
            <div className="flex gap-3">
