...
import { Check, MessageSquare, AlertCircle, User, Search, PlusCircle, ArrowLeft } from "lucide-react";
...
export const RecordPage = () => {
  ...
  // New state for smart search flow
  ...
  // Auto-focus inputs when switching to "new" mode
  useEffect(() => {
    if (mode === "new") {
      if (!tx.name) nameInputRef.current?.focus();
  ...
  const searchResults = useMemo(() => {
    if (!searchQuery.trim()) return customers.slice(0, 5); // Show recent 5 if empty
  ...
  // Google-style inline creation
  const handleCreateInline = () => {
    const isPhone = /^\d+$/.test(searchQuery.replace(/\s/g, ''));
    if (isPhone) {
      setTx(prev => ({ ...prev, phone: searchQuery, customerId: null }));
    } else {
      setTx(prev => ({ ...prev, name: searchQuery, customerId: null }));
    }
    setMode("new");
    setSearchQuery("");
  };
  ...
  const saveTransaction = async () => {
    ...
    try {
      ...
      resetToSearch(); // Reset to search mode for next entry
      setView("home");
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-24">
      ...
      <div className="p-4 space-y-4 max-w-lg mx-auto">
            ...
            <div className="mt-3 space-y-2 max-h-60 overflow-y-auto">
              {searchResults.map(c => (
                ...
                >
                  ...
                  {c.balance > 0 && (
                    <span className="text-xs font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded-full flex-shrink-0">
              ...
              {/* Inline Create Button (Google Style) */}
              {searchQuery.trim() && searchResults.length === 0 && (
                <button 
                  onClick={handleCreateInline}
                  className="w-full flex items-center gap-3 p-3 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 transition text-left"
                >
        ...
        {mode === "new" && (
          <div className="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 space-y-3">
            <div className="flex items-center justify-between mb-1">
              ...
              <button 
                onClick={resetToSearch} 
                className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 hover:text-green-600"
              >
        ...
        {/* TRANSACTION DETAILS (Only show if customer is selected/created) */}
