...
// 👇 CHANGED: Accept initialData prop for pre-filling
export const AddCustomerModal = ({ onClose, initialData = {} }) => {
  const { currentStore, refreshCustomers, showToast } = useStore();
  const [form, setForm] = useState({ name: "", phone: "", altPhone: "", address: "", notes: "" });
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState("");

  // 👇 NEW: Pre-fill form if initialData is provided
  useEffect(() => {
  ...
  const handlePickContact = async () => {
    ...
    try {
      ...
      if (contact && contact.tel && contact.tel[0]) {
        ...
        setForm(prev => ({ ...prev, phone: phoneNum })); setError("");
      }
  ...
  const handleSave = async () => {
    ...
    const existing = await CustomerRepository.getByPhone(currentStore.id, form.phone);
    if (existing) { setError("A customer with this phone number already exists."); return; }

    setIsSaving(true);
    try {
      await CustomerRepository.add(currentStore.id, {
        name: form.name.trim(),
    ...
    <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-5 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-900 z-10">
          <h3 className="font-bold text-xl text-gray-900 dark:text-white">Add New Customer</h3>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full pl-10 pr-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-green-500 dark:text-white" autoFocus />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input type="tel" inputMode="tel" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} className="w-full pl-10 pr-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-green-500 dark:text-white" />
              </div>
              <button type="button" onClick={handlePickContact} className="flex-shrink-0 p-3 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-xl active:scale-90"><Contact size={20} /></button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <Phone className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
              <input type="tel" inputMode="tel" value={form.altPhone} onChange={e => setForm({...form, altPhone: e.target.value})} className="w-full pl-10 pr-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-green-500 dark:text-white" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} rows="2" className="w-full pl-10 pr-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-green-500 dark:text-white resize-none" />
            </div>
          </div>
        </div>
        ...
        <div className="p-5 border-t border-gray-200 dark:border-gray-700 sticky bottom-0 bg-white dark:bg-gray-900">
          <button onClick={handleSave} disabled={isSaving} className="w-full bg-green-700 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 disabled:opacity-50">
            {isSaving ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <><Save size={18} /> Save Customer</>}
          </button>
        </div>
      </div>
    </div>
