...
export const InvoiceModal = ({ onClose, transaction, customerName, customerPhone }) => {
  ...
  // 👇 1. Get dynamic currency from store
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-md w-full shadow-2xl overflow-hidden max-h-[90vh] flex flex-col print:rounded-none print:shadow-none print:max-h-none print:w-full print:overflow-visible">
        ...
        <div className="flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 print:hidden">
          <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <FileText size={18} className="text-green-600" /> Receipt Details
          </h3>
          {/* 👇 2. Obvious Close Button in Header */}
          <button onClick={onClose} className="p-2 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-full hover:bg-red-200 dark:hover:bg-red-900/50 transition" title="Close">
            <X size={20} />
          </button>
        </div>
          ...
          <div className="p-6 space-y-4">
                    ...
                    <div key={index} className="flex justify-between items-center text-sm border-b border-gray-100 dark:border-gray-800 pb-2 last:border-0">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">
                          {item.quantity} {item.unit ? `× ${item.unit}` : 'pcs'} @ {formatCurrency(item.price, currency)}
                        </p>
                      </div>
            ...
            <div className="pt-4 border-t-2 border-gray-200 dark:border-gray-700 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-gray-900 dark:text-white">{formatCurrency(transaction.amount, currency)}</span>
              </div>
              {transaction.paid > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">Amount Paid</span>
                  <span className="font-semibold text-green-600 dark:text-green-400">-{formatCurrency(transaction.paid, currency)}</span>
                </div>
              )}
              <div className="flex justify-between items-center pt-3 border-t border-dashed border-gray-300 dark:border-gray-600">
                ...
                <span className={`text-2xl font-bold font-mono ${balanceDue > 0 ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"}`}>
            ...
            <div className="pt-6 text-center pb-4">
              ...
              {transaction.isVoid && (
                <div className="mt-4 border-2 border-red-500 text-red-500 font-bold text-xl py-2 px-4 rounded-lg transform -rotate-12 inline-block opacity-80">VOIDED</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 flex gap-3 print:hidden">
          <button onClick={handlePrint} className="flex-1 flex items-center justify-center gap-2 bg-blue-600 text-white font-bold py-3 rounded-xl hover:bg-blue-700 transition">
            <Printer size={18} /> Print
          </button>
          {/* 👇 4. Big Obvious Close Button at the Bottom */}
          <button onClick={onClose} className="flex-1 flex items-center justify-center gap-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-bold py-3 rounded-xl hover:bg-gray-300 dark:hover:bg-gray-600 transition">
            Close
          </button>
        </div>
      </div>
