...
export const generateAccountShare = ({ channel, scope, contact, transactions, store, customRange }) => {
  ...
  if (scope === 'last1') {
    scopedTx = sortedTx.slice(-1); // 👇 NEW: only the most recent transaction
  } else if (scope === 'last5') {
  ...
  } else if (scope === 'custom' && customRange && customRange.startDate && customRange.endDate) {
    // 👇 NEW: exact date window (inclusive of both days)
    const start = new Date(customRange.startDate); start.setHours(0, 0, 0, 0);
    const end = new Date(customRange.endDate); end.setHours(23, 59, 59, 999);
    scopedTx = sortedTx.filter(tx => {
      const d = new Date(tx.created_at || tx.createdAt || 0);
  ...
  if (scope === 'custom' && customRange && customRange.startDate && customRange.endDate) {
    L.push(`📅 Period: ${customRange.startDate} → ${customRange.endDate}`);
  }
  L.push("");
  ...
  if (scope !== 'balance') {
    if (scopedTx.length === 0) {
      L.push("📭 No transactions found in the selected period.");
      L.push("");
    }
    scopedTx.forEach(tx => {
